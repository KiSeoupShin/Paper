...
def bbox_attr(data, i):
    '''
    각 bounding box의 i번째 원소를 반환
    '''

    attr_start = config.C + i
    return data[..., attr_start::5] # ... -> :,: 과 같이 슬라이싱이 많을 때 한번에 처리하고자 사용
...
def get_iou(p, a):  # p:predict, a:actual
    p_bl, p_tr = bbox_to_coords(p)     # (batch, S, S, B, 2)
    a_bl, a_tr = bbox_to_coords(a)     # bl:bottom_left, tr:top_right

    # 가장 큰 top_right와 가장 작은 bottom_left를 ...?
    coords_join_size = (-1, -1, -1, config.B, config.B, 2)
    tr = torch.max(
        p_tr.unsqueeze(4).expand(coords_join_size),    # (batch, S, S, B, 1, 2) -> (batch, S, S, B, B, 2)
        a_tr.unsqueeze(3).expand(coords_join_size)     # (batch, S, S, 1, B, 2) -> (batch, S, S, B, B, 2)
    )
    bl = torch.min(
        p_bl.unsqueeze(4).expand(coords_join_size),
        a_bl.unsqueeze(3).expand(coords_join_size)
    )

    intersection_sides = torch.clamp(bl - tr, min=0.0)
    ...
    a_area = a_area.unsqueeze(3).expand_as(intersection)      # (batch, S, S, B, 1) -> (batch, S, S, B, B)

    union = p_area + a_area - intersection

    # division-by-zero 오류 조정
...
def plot_boxes(data, labels, classes, color='orange', min_confidence=0.2, max_overlap=0.5, file=None):
    ...
    for i in range(m):
        for j in range(n):
            for k in range((labels.size(dim=2) - config.C) // 5):
                ...
                confidence = labels[i, j, class_index].item() * bbox[4].item()
    ...
    for i in range(num_boxes):
        if i not in discarded:
            ...
            text = f'{classes[class_index]} {found(confidence * 100, 1)}%'
